...
class LayerwiseProfiler():
    def __init__(self, layerwise_names):
        self.layerwise_names = layerwise_names 
        self.layerwise_times = [0]*len(layerwise_names)
        self.layer_to_time = {}
        for name in layerwise_names:
            self.layer_to_time[name] = []
        self.first_begin_timestamps = []
        self.current_timestamp = time.time()

    def start(self):
        self.first_begin_timestamps.append(time.time())

    def layer_begin(self, name):
        torch.cuda.current_stream().synchronize()
        self.current_timestamp = time.time()
        self.layer_to_time[name].append(self.current_timestamp)

    def layer_end(self, name):
        torch.cuda.current_stream().synchronize()
        self.current_timestamp = time.time()
        self.layer_to_time[name].append(self.current_timestamp)
    ...
    def get_results(self):
        avg_layerwise_times = []
        current_layer_data = np.array(self.first_begin_timestamps)
        for name in self.layerwise_names:
            d = np.array(self.layer_to_time[name])
            avg_layerwise_times.append(np.mean(d-current_layer_data))
            current_layer_data = d
        return avg_layerwise_times 
