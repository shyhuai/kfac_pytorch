    ...
    def step(self, closure=None, epoch=None):
        ...
        if self.steps % self.fac_update_freq == 0:
            ...
            if self.eigen_ranks is None:
                if not self.exclude_compute_factor:
                    self._update_A()
                    self._update_G()
                self.eigen_ranks, self.fusion_groups = self._generate_eigen_ranks_blockpartition_opt(epoch)
                if not self.exclude_communicate_factor:
                    if hvd.size() > 1:
                ...
                if hvd.rank() == 0:
                    ...
                    print('fusion_groups: ', self.fusion_groups)

                self.fw_merged_comm.update_tensor_fusion(self.fusion_groups)
        ...
        if hvd.size() > 1 and self.steps % self.kfac_update_freq == 0:
            if self.steps == 5:
                ...
                self.fw_merged_comm.update_groups(self.fusion_groups, fw_factor_sizes, fw_layerwise_times, reverse=False)
                #self.bw_merged_comm.update_groups(self.fusion_groups, bw_factor_sizes, bw_layerwise_times, reverse=True)
                self.profiling = False  
